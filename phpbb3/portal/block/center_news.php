...
/*
*
* @name center_news.php
* @package phpBB3 Portal XL
* @version $Id: center_news.php,v 1.4 2011/10/10 portalxl group Exp $
*
	...
	*/
	...
	if($news < 0)
	{
		for ($i = 0; $i < count($fetch_news); $i++)
		{
			...
			// last comments
			$center_news_last_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . $fetch_news[$i]['topic_last_post_id']) . '#p' . $fetch_news[$i]['topic_last_post_id'];
			// topic id
			$center_news_topic_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id']) . '#p' . $fetch_news[$i]['post_id'];
			...
			$template->assign_block_vars('news_row', array(
				...
				'U_LAST_COMMENTS'			=> $center_news_last_post_url,
				...
				'U_VIEW_COMMENTS'			=> $center_news_topic_url,
				));						
	...
	else
	{
		...
		*/
		...
		$unread_topic = (isset($topic_tracking_info[$topic_id]) && $fetch_news[$i]['topic_last_post_time'] > $topic_tracking_info[$topic_id]) ? true : false; 
		// last comments
		$last_post_center_news_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$fetch_news[$i]['topic_id']]) ) . '#p' . $fetch_news[$i]['topic_last_post_id'];
		// topic id
		$topic_center_news_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$fetch_news[$i]['topic_id']]) ) . '#p' . $fetch_news[$i]['post_id'];
		...
		$template->assign_block_vars('news_row', array(
			...
			'TOPIC_VIEWS'				=> $fetch_news[$i]['topic_views'],
			'U_VIEW_UNREAD'		        => append_sid("{$phpbb_root_path}viewtopic.$phpEx", (($real_forum_id) ? 'f=' . $real_forum_id . '&amp;t=' : '') . $fetch_news[$i]['topic_id'] . '&amp;view=unread#unread'),
			'U_LAST_COMMENTS'			=> $last_post_center_news_url,
			'U_POST_COMMENT'			=> append_sid("{$phpbb_root_path}posting.$phpEx", 'mode=reply&amp;' . (($real_forum_id) ? 'f=' . $real_forum_id . '&amp;' : '') . 't=' . $topic_id),
			'U_READ_FULL'      			=> append_sid("{$phpbb_root_path}portal.$phpEx", $read_full_url),
			'U_USER_PROFILE'			=> (($fetch_news[$i]['user_type'] == USER_NORMAL || $fetch_news[$i]['user_type'] == USER_FOUNDER) && $fetch_news[$i]['user_id'] != ANONYMOUS) ? append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=viewprofile&amp;u=' . $fetch_news[$i]['user_id']) : '',
			'U_VIEWFORUM'				=> append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $fetch_news[$i]['forum_id']),
			'U_VIEW_COMMENTS'			=> $topic_center_news_url,
			));
...
$template->set_filenames(array(
    'body' => 'portal/block/center_news.html',
	));
