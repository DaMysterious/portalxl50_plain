...
global $phpbb_root_path, $phpEx, $config, $portal_config;
	...
	*/
	...
	{
		if (preg_match('#([^.]+)\.#', $row_portal_blocks['block_name'], $forumMatch))
		if (file_exists($phpbb_root_path . 'portal/block/'.$forumMatch[1].'.'.$phpEx))
		include_once($phpbb_root_path . 'portal/block/'.$forumMatch[1].'.'.$phpEx);
	}
	$db->sql_freeresult($result);
