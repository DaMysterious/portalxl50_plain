...
/*
*
* @name scroll_actual_recent_topics.php
* @package phpBB3 Portal XL 5.0
* @version $Id: scroll_actual_recent_topics.php,v 1.4 2009/10/13 portalxl group Exp $
*
...
while( ($row = $db->sql_fetchrow($result)) && ($row['topic_title'] != '') )
{
	...
	{
		// last comments
		$scroll_actual_recent_topics_last_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$row['topic_id']]) ) . '#p' . $row['topic_last_post_id'];
		// topic id
		$scroll_actual_recent_topics_topic_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id'] . '&amp;t=' . $row['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$row['topic_id']]) ) . '#p' . $row['post_id'];

		$row['username'] = '<b style="color:#' . $row['user_colour'] . '">' . $row['username'] . '</b> ';

			$template->assign_block_vars('scroll_actual_recent_topics', array( 
				'U_TITLE' => $scroll_actual_recent_topics_topic_url,
...
$template->set_filenames(array(
    'body' => 'portal/block/scroll_actual_recent_topics.html',
	));
