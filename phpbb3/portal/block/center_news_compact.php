...
/*
*
* @name center_news_compact.php
* @package phpBB3 Portal XL
* @version $Id: center_news_compact.php,v 1.4 2011/10/10 portalxl group Exp $
*
...
{
	$template->assign_block_vars('news_compact_row', array(
		'S_NO_TOPICS'	=> true,
		'S_NOT_LAST'	=> false
		));
	...
	*/
	...
	if($news < 0)
	{
		for ($i = 0; $i < count($fetch_news); $i++)
		{
			...
			// last comments
			$center_news_compact_last_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . $fetch_news[$i]['topic_last_post_id']) . '#p' . $fetch_news[$i]['topic_last_post_id'];
			// topic id
			$center_news_compact_topic_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id']) . '#p' . $fetch_news[$i]['post_id'];
			...
			$template->assign_block_vars('news_compact_row', array(
				...
				'U_LAST_COMMENTS'			=> $center_news_compact_last_post_url,
				...
				'U_VIEW_COMMENTS'			=> $center_news_compact_topic_url,
				));						
			...
			if( !empty($fetch_news[$i]['attachments']) )
			{
				...
				{
					$template->assign_block_vars('news_compact_row.attachment', array(
	...
	else
	{
		...
		*/
		...
		$unread_topic = (isset($topic_tracking_info[$topic_id]) && $fetch_news[$i]['topic_last_post_time'] > $topic_tracking_info[$topic_id]) ? true : false; 
		// last comments
		$center_news_compact_last_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$fetch_news[$i]['topic_id']]) ) . '#p' . $fetch_news[$i]['topic_last_post_id'];
		// topic id
		$center_news_compact_topic_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $fetch_news[$i]['forum_id'] . '&amp;t=' . $fetch_news[$i]['topic_id'] . '&amp;start=' . @intval($phpbb_seo->seo_opt['topic_last_page'][$fetch_news[$i]['topic_id']]) ) . '#p' . $fetch_news[$i]['topic_first_post_id'];
...
$template->set_filenames(array(
    'body' => 'portal/block/center_news_compact.html',
	));
