...
* @version $Id: acp_portal_banners_ve.php,v 1.3 2009/10/18 portalxl group Exp $
*
...
class acp_portal_banners_ve
{
	...
	function main($id, $mode)
	{
		...
		$this->tpl_name = 'portal_xl/acp_portal_banners_ve';
		$this->page_title = 'ACP_PORTAL_BANNERS_VE';
		...
		case 'save':
				...
				if ($banners_id)
				{
				    $sql = 'UPDATE ' . PORTAL_BANNER_VE_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . " WHERE banners_id = $banners_id";
				    $message = $user->lang['BANNERS_UPDATED'];
				}
				else
				{
				     $sql = 'INSERT INTO ' . PORTAL_BANNER_VE_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
			...
			case 'delete':
				...
				if ($banners_id)
				{
					$sql = 'DELETE FROM ' . PORTAL_BANNER_VE_TABLE . "
			...
			case 'add':
				...
				$template->assign_vars(array(
					...
					'BANNERS_IMG'			=> (isset($banners_img_ver['banners_img_ver'])) ? $banners_img_ver['banners_img_ver'] : '')
				);
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$template->assign_block_vars('banners', array(
				...
				'BANNERS_IMG' 			=> '<div style="margin:2px;"><img src="../' . $row['banners_img_ver'] . '" width="75" height="300" alt="' . $row['banners_description'] . '" title="' . $row['banners_description'] . '" /></div>',
